...
<body> 
<form action="${pageContext.request.contextPath }/updateItemsave.action" method="post">
查询条件：
...
<td><input type="text" name="updateItemsList[${status.index}].createtime" value="<fmt:formatDate value="${item.createtime}" pattern="yyyy-MM-dd HH:mm:ss"/>"/></td>
	...
	<td><input type="hidden" name="updateItemsList[${status.index}].id" value="${item.id }"/><a href="${pageContext.request.contextPath }/itemEdit.action?id=${item.id}">修改</a></td>
